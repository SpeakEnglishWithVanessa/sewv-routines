...
- **Promo Library (Google Drive folder)** — reusable, date-stripped templates for promos that run more than once a year (e.g. SAVE70). Any promo doc built as a reusable template (as opposed to a one-off, single-run promo) always gets saved here, not left in whatever folder its source material happened to be in. When a recurring promo runs again, pull the matching template from this folder, fill in the new date window, and use it to build the current 08-current-promo.md for the knowledge base project — see file 00's "Drafting priority order" for how file 08 is used day-to-day.  
