...
- **Promotions calendar spreadsheet** — master promo calendar (Google Sheets, tab per year). Live reference to check, not something to copy into this knowledge base: [https://docs.google.com/spreadsheets/d/11bsBRUw6\_3u6CydUq9Fg9EnLRaQXZz04Pxz-wdkoE0I/edit](https://docs.google.com/spreadsheets/d/11bsBRUw6_3u6CydUq9Fg9EnLRaQXZz04Pxz-wdkoE0I/edit)  
- **Promo Library (Google Drive folder)** — holds reusable templates for promos that recur (run more than once a year, e.g. Speak With Vanessa AI SAVE70, the 150 Day Listening Challenge Bundle). Each entry has the fixed terms (price, links, mechanics, eligibility) with blank date fields to fill in on each new run. When a recurring promo starts again, pull its template from here, fill in the current dates, and rebuild file 08 (or 09, for a second concurrent promo) from it rather than starting from scratch. One-off/first-time promos won't have a template yet — worth creating one the first time any promo runs, so the next run is a fill-in-the-dates job instead of a rebuild.  
- **Canva** — completion certificates.  
- [**hi@yourteacher.ai**](mailto:hi@yourteacher.ai) **(Peter)** — external paid support for Vanessa AI app issues; route bugs here, don't troubleshoot internally.  
- **Course sign-in page**: [https://speak-english-with-vanessa.teachable.com/sign\_in](https://speak-english-with-vanessa.teachable.com/sign_in)
...
- **2026 Study Plan**: [https://drive.google.com/file/d/1RQmK\_z48Y83lfJxSoP\_GxJgfqzi5nmmh/view?usp=sharing](https://drive.google.com/file/d/1RQmK_z48Y83lfJxSoP_GxJgfqzi5nmmh/view?usp=sharing) — names the current featured module with its direct link. Safe to send straight to a student.
...
- **Claude/Possible unsubscribe** — explicit "stop emailing me" signals, including plain-language phrasing without the word "unsubscribe" itself (e.g. "stop your gmail," "please stop emailing me," "stop sending me emails") — most ESL students won't use the word "unsubscribe." **No customer-facing reply is drafted or sent for these, regardless of tone (polite or rude)** — label only, then unsubscribe via Kit (Procedure O). Not a Claude/Need admin (high priority) case — that label is for things needing a real account lookup (refunds, enrollment checks, coupon creation); a plain unsubscribe is a single simple action and doesn't need the "high priority" admin treatment. See file 01 \#20b.  
...
1. **The admin checklist first** — to: [contact@speakenglishwithvanessa.com](mailto:contact@speakenglishwithvanessa.com) as a safe placeholder (the Gmail draft tool rejects a fully empty "to" field), subject "🔧 ADMIN TASKS — DO NOT SEND," body containing the fully-written-out steps (not a pointer to a procedure number) with that student's actual details filled in.  
...
- **Admin Archive** (and nested sub-labels) — reference/record-keeping, not regularly checked:  
  ...
  - **Vanessa AI** — largely obsolete; Vanessa-AI support questions now route straight to Peter ([hi@yourteacher.ai](mailto:hi@yourteacher.ai)) instead.  
...
For non-course, ongoing "help me with my English" requests unrelated to a specific lesson, see file 01 entry \#29 — email support scope is limited to course-joining and lesson-content questions, not open-ended tutoring.
...
**Self-sent newsletter copies** (sender \= recipient \= [contact@speakenglishwithvanessa.com](mailto:contact@speakenglishwithvanessa.com), no third-party involved) — always apply the full Claude/Need deleted label, don't just clear the unread status without labeling.
...
**"5-Minute English" — two unrelated things, don't conflate them:**

1. **Welcome Sequence broadcast titles** (items 14–18 above, "5-Minute English: In a heartbeat" etc.) — fixed, never-changing subject lines sent to newsletter subscribers pre-purchase. Unrelated to course content.  
2. **FFC member daily lesson sequence** (Mon–Fri, active/paying members only) — a short daily snippet tied to whatever module/lesson the club is currently studying, so members can study a little each day even without logging in. Has its own dedicated opt-out link, separate from the general newsletter unsubscribe link.

**How to tell which one a reply is quoting, when in doubt:** the Welcome Sequence subject lines never change, so if the quoted original's subject matches one of the 24 fixed titles above, it's that sequence. The definitive tell for the member daily sequence is its footer: *"You're receiving these daily lessons because you are a member of The Fearless Fluency Club. If you don't want to receive these daily lessons, click here to stop receiving them (but you will still be a member of the course)."* That members-only footer line is the reliable signal — don't rely on the general newsletter footer/unsubscribe link or on whether "the club" is mentioned by name, since both sequences reference FFC and share the same general newsletter footer. If the quoted original isn't visible or is ambiguous, fall back to the standard Kit membership-tag check (see "Verifying FFC membership before assuming it," above).

**"I stopped receiving the 5-minute emails" (from a confirmed active FFC member):** almost always means they clicked the sequence-specific opt-out link, not a technical failure or full unsubscribe. **Route to Claude/Need admin** — go into Kit and remove that opt-out flag so the sequence resumes. This is a known, fixable membership-email issue, not a delete.
...
**Recognizing "lesson guide" requests by module theme name** — when a student names a themed module (e.g. "the San Diego lesson," "the adoption lesson") and asks for a PDF/guide, they're asking for that module's lesson guide, found **inside Teachable within the lesson itself**: log in at [https://speak-english-with-vanessa.teachable.com/sign\_in](https://speak-english-with-vanessa.teachable.com/sign_in), open the named lesson, and click the lesson guide link at the top of the page — not the general Worksheet Library, not a "check your confirmation email" response, and **not** the Facebook group "FILES" section (that's where the separate weekly review PDF worksheet lives, a different asset — see file 05).
...
**"Re: A quick question" — check content, not just subject.** Multiple different Kit broadcasts share similar subject lines. One variant to watch for: "What topic would you love to study in The Fearless Fluency Club?" (sent to current FFC students, asking what other topics they'd like the club to cover — recognizable from the quoted original by the 🎵 Music? / 🏦 Business? / 🌮 Restaurants? / Any others? list). **Every reply to this variant gets a stock reply** (regardless of how many topics are named, 1 or many) and is labeled **VANESSA/Vanessa Informational** — not Claude/Need deleted. See the stock reply template in file 06\. (This supersedes the older 4+-topics-to-flag-Vanessa / 1–3-topics-to-delete threshold — that rule is retired.)
...
6. **`update_draft` threading bug is the norm, not the exception** — see file 07\. As of early August 2026, every use of `update_draft` on an existing draft observed in practice has detached it onto an orphaned thread. Continue using the create\_draft \+ replyToMessageId \+ DISREGARD-the-old-one workaround by default; worth flagging to whoever manages the Gmail connector as a live bug rather than a rare edge case.
